...
namespace Workflows.Functions
{
    public static class ActivityFunctions
    {
            ...
            ILogger log)
        {
            ...
            var host = Environment.GetEnvironmentVariable("WEB_HOST");

            var approveUrl = $"{host}/admin.html";
        ...
        [FunctionName("A_EnqueueOrder")]
        [return: Queue("orders")]
        public static string EnqueueOrder(
                            [ActivityTrigger] (Order, OrderItem) orderInfo,
                            ILogger log)
        {
            var (order, item) = orderInfo;
            log.LogInformation("Sending order to queue");
            if (item.ProductId == "error")
                throw new InvalidOperationException("Not enqueuing the product");

            return $"Order: ${item.ProductId} for {order.PurchaserEmail}";
        }
    }
}