...
var app = new Vue({
    ...
    methods: {
        getOrderStatuses: function () {
            ...
            fetch('http://localhost:7071/api/getallorders/')
        ...
        approve: function (order, status) {
            postData(`http://localhost:7071/api/approve/${order.input.Id}`, status)
                .then(_ => order.customStatus = '');
        },
        deleteOrder: function (order) {
            fetch(`http://localhost:7071/api/order/${order.input.Id}`, { method: 'DELETE' })
                .then(_ => {
